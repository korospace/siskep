...
<?= $this->section('dashboardJs'); ?>
  <script>
    <?php if ($previlege != 'nonasn') { ?>
      // get list of pegawai
      let arrUser = [];
      let optionPegawai1 = "<option value=''>-- pilih pegawai --</option>";
      let optionPegawai2 = "<option value=''>-- semua pegawai --</option>";
      
      async function getDataPegawai() {
          optionPegawai1 = "<option value=''>-- pilih pegawai --</option>";
          optionPegawai2 = "<option value='semua pegawai'>-- semua pegawai --</option>";
          ...
          if (httpResponse.status === 200) {
              ...
              arrUser.forEach(e => {
                  if (e.previlege == "nonasn") {
                    optionPegawai1 += `<option value="${e.id}">${e.nama_lengkap}</option>`;
                    optionPegawai2 += `<option value="${e.id}">${e.nama_lengkap}</option>`;
                  }
              });
          }

          $("#user_id_tugas").html(optionPegawai1);
          $("#pegawai_filtug").html(optionPegawai2);
      }
      getDataPegawai();
    <?php } ?>
  </script>
              ...
              <div
                class="mt-4 pt-4 md:pt-0 border-t md:border-none border-zinc-300">
                  <div class="flex items-center">
                    <div id="btn_filter" class="w-max mr-3 px-1.5 pt-1.5 pb-1 bg-white shadow-md rounded-sm cursor-pointer transition transform active:scale-90" onclick="showFilterTugas();">
